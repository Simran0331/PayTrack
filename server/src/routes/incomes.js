...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    const { amount, category, description, date } = createIncomeSchema.parse(req.body);
    ...
    if (e?.code === 11000) {
      const existing = await Income.findOne({ user: req.userId, idempotencyKey }).lean();
...
router.get('/', requireAuth, async (req, res) => {
  ...
  const items = await Income.find(filter).sort(sortObj).lean();
